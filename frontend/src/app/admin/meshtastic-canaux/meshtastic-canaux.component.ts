...
/** Chat Meshtastic unifié — même principe que MeshcoreCanauxComponent (MeshCore), avec une
 * différence de taille : un DM Meshtastic est chiffré avec la PSK d'un CANAL partagé (pas une
 * clé propre au destinataire, voir doc de conception meshtastic-bridge/README.md), il faut donc
 * choisir explicitement lequel à l'envoi — pas seulement le destinataire. */
@Component({
...
export class MeshtasticCanauxComponent implements OnInit, OnDestroy {
  ...
  /** Canal dont la PSK sera utilisée pour chiffrer le prochain DM envoyé — n'a de sens que
   * pour une conversation de type 'dm'. */
  canalPourEnvoiId = '';
  ...
  envoyer(): void {
    ...
    if (conv.type === 'canal') {
      ...
      if (!this.compagnonPrincipalId || !this.canalPourEnvoiId) {
        this.errorMessage = 'Aucun companion ou canal disponible pour envoyer ce message.';
        this.envoiEnCours = false;
        return;
      }
      this.dmService.envoyer(this.compagnonPrincipalId, this.canalPourEnvoiId, conv.noeud.node_num, contenu).subscribe({
        next: (message) => {
